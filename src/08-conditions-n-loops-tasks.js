...
function isInsideCircle(circle, point) {
  ...
  if (((point.x - circle.center.x) ** 2) + ((point.y - circle.center.y) ** 2) < rad2) {
    return true;
  }
  return false;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((acc, d) => acc + +d, 0));
}
...
function getCommonDirectoryPath(pathes) {
  ...
  while (i < pathArr.length) {
    ...
    for (let j = 0; j < pathes.length; j += 1) {
      const arr = pathes[j].split('/');
      if (arr[i] !== path) return str;
    }
    str += `${path}/`;
    i += 1;
  }
  ...
  for (let j = 0; j < m2[0].length; j += 1) {
    m1.forEach((item, index) => {
      item.forEach((item2, index2) => {
        matrix[index][j] += item2 * m2[index2][j];
      });
    });
  }
  return matrix;
